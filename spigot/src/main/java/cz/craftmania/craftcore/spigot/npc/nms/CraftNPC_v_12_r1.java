package cz.craftmania.craftcore.spigot.npc.nms;
...
import cz.craftmania.craftcore.spigot.npc.events.CraftNPCDespawnEvent;
import cz.craftmania.craftcore.spigot.npc.events.CraftNPCSpawnEvent;
import cz.craftmania.craftcore.spigot.npc.skin.*;
import cz.craftmania.craftcore.spigot.utils.StringUtils;
...
public class CraftNPC_v_12_r1 implements CraftNPC {

    private static List<CraftNPC_v_12_r1> npcs = new ArrayList<CraftNPC_v_12_r1>();
    ...
    public static void startTask(Plugin plugin){
        if(!taskstarted){
            taskstarted = true;
            CraftNPC_v_12_r1.plugin = plugin;
            Bukkit.getScheduler().scheduleSyncRepeatingTask(plugin, new Runnable() {
                @Override
                public void run() {
                    for(CraftNPC_v_12_r1 nmsnpc : npcs){
    ...
    private JsonSkinData getCachedSkin(){
        if(CraftNPCApi.getCache() && this.skin.getSkinType()!= SkinType.PLAYER) {
    ...
    public CraftNPC_v_12_r1(Location location, CraftNPCSkin skin){
        npcid = id++;
        this.skin = skin;
    ...
    private void spawnEnttity(Player p){
        ...
        Bukkit.getScheduler().runTaskLater(CraftNPC_v_12_r1.plugin, new Runnable(){
            @Override
            public void run() {
        ...
        CraftNPCSpawnEvent event = new CraftNPCSpawnEvent(p, (CraftNPC) this);
    ...
    private void destroy(Player p){
        ...
        try{
            ...
            CraftNPCDespawnEvent event = new CraftNPCDespawnEvent(p, (CraftNPC) this);
            Bukkit.getPluginManager().callEvent(event);
        }catch(Exception ex){
            ex.printStackTrace();
        }
