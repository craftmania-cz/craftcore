...
import cz.wake.craftcore.bungee.annotations.PacketHandler;
import cz.wake.craftcore.bungee.utils.ReflectionFix;
...
public class PacketListener implements Listener {
    ...
    public class Handler{
        ...
        public Object getPacketValue(String field){
            return ReflectionFix.getField(field, packet.getClass(), packet);
        }

        public void setPacketValue(String field, Object value){
            ReflectionFix.setField(field, packet.getClass(), packet, value);
        }
    }
    ...
    public static Channel getChannel(ProxiedPlayer player) {
        ...
        ChannelWrapper cw = (ChannelWrapper) ReflectionFix.getField("ch", uc.getClass(), uc);
        return (Channel) ReflectionFix.getField("ch", cw.getClass(), cw);
    }
    ...
    private void init(ProxiedPlayer player) {
        ...
        Channel channel = getChannel(player);
        if (channel.pipeline().get(PACKET_HANDLER) == null) {
            int t = BungeeCord.getInstance().getScheduler().runAsync(Main.instance, new Runnable() {
                @Override
                public void run() {
